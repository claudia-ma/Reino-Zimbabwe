<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nuevo testimonio</h2>
            <a href="{{ route('admin.testimonios.index') }}" class="text-sm text-gray-600 hover:text-gray-900">
            ...
            <form method="POST" action="{{ route('admin.testimonios.store') }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="estrellas" required
                                class="mt-1 w-full rounded-lg border-gray-300 focus:border-gray-900 focus:ring-gray-900">
                            @for($i=5; $i>=1; $i--)
                                <option value="{{ $i }}" @selected(old('estrellas', 5) == $i)>{{ $i }}/5</option>
                            @endfor
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="mensaje" rows="4" required
                              class="mt-1 w-full rounded-lg border-gray-300 focus:border-gray-900 focus:ring-gray-900">{{ old('mensaje') }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Foto URL (opcional)</label>
                    <input name="foto_url" value="{{ old('foto_url') }}"
                           class="mt-1 w-full rounded-lg border-gray-300 focus:border-gray-900 focus:ring-gray-900">
                </div>

                <div class="flex items-center gap-2">
                    <input type="checkbox" name="publicado" value="1" {{ old('publicado', true) ? 'checked' : '' }}
